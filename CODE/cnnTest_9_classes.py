#  10-fold CV of CNN on the dataset with 9 classes
#  HVHA_0, HVLA_0, LVHA_0 and LVLA_0 classes are combined into one class BASE
# 'BASE', 'HVHA_1', 'HVHA_2', 'HVLA_1', 'HVLA_2', 'LVHA_1', 'LVHA_2','LVLA_1', 'LVLA_2'
...
X = np.concatenate((class1, class4, class7, class10, class2, class3, class5, class6, class8, class9, class11, class12),
                   axis=3)
...
y1 = 0 * np.ones((class1.shape[3] + class4.shape[3] + class7.shape[3] + class10.shape[3]))
y2 = 1 * np.ones((class2.shape[3]))
y3 = 2 * np.ones((class3.shape[3]))
y5 = 3 * np.ones((class5.shape[3]))
y6 = 4 * np.ones((class6.shape[3]))
y8 = 5 * np.ones((class8.shape[3]))
y9 = 6 * np.ones((class9.shape[3]))
y11 = 7 * np.ones((class11.shape[3]))
y12 = 8 * np.ones((class12.shape[3]))

y = np.concatenate((y1, y2, y3, y5, y6, y8, y9, y11, y12), axis=0).astype(int)
...
classNames = ['BASE', 'HVHA_1', 'HVHA_2', 'HVLA_1', 'HVLA_2', 'LVHA_1', 'LVHA_2', 'LVLA_1', 'LVLA_2']
n_classes = 9
nfold = 10
...
with plt.style.context("default"):
    ...
    axs[0].set_xticklabels(classNames, fontsize=10)
    ...
    axs[1].set_xticklabels(classNames, fontsize=10)
    axs[1].set_ylabel('Number of test instances')
    plt.tight_layout()
    plt.savefig('FIGURES/trainTestDist_cnn_10fold_9_classes.pdf')
...
# Save accuracy perr fold per epoch
...
with open(accTotal_9_fn, 'wb') as fp:
    pickle.dump(acc_mean, fp)

reports_9_fn = ('reportsCNN_9.p')
with open(reports_9_fn, 'wb') as fp:
    pickle.dump(reports, fp)

confusionMatrices_9_fn = ('confusionMatricesCNN_9.p')
with open(confusionMatrices_9_fn, 'wb') as fp:
...
with plt.style.context("default"):
    ...
    plt.grid
    plt.savefig('FIGURES/accTrainVal_10foldCV_9classes.pdf')
...
plt.tight_layout()
plt.savefig('FIGURES/normCM_cnn_10fold_9_classes.pdf')
...
with plt.style.context("default"):
    ...
    plt.tight_layout()
    plt.savefig('FIGURES/accuracyTrainVal_cnn_10fold_9_classes.pdf')
